...
for j=1:length(File)
    ...
    for i=1:nele
        ...
        keyword     = {' HISTORY FOR ELEMENT   2 OF ELEMENT GROUP   3, ANALYSIS SEGMENT   2'};
        toggle      = true;
        ...
        while toggle
            ...
            if length(aline)>=length(char(keyword))
                if length(aline)>0 && strcmpi(aline(1:length(char(keyword))),keyword)
                    ...
                    extract     = regexp(aline,exp,'match');
                    % Node 1
                    elemBRA{j}{i}{1}(1,3)  = str2double(extract(7));
                    % Node 2
                    elemBRA{j}{i}{2}(1,3)  = str2double(extract(8));
                    for k=1:4
                        aline   = fgetl(opf);
                    end
                    toggle    = false;
        ...
        for p=1:nstp
            ...
            if length(aline)>0
                ...
                elemBRA{j}{i}{1}(p,2)  = str2double(extract(2))*scalef;
                elemBRA{j}{i}{1}(p,3)  = elemBRA{j}{i}{1}(1,3);
                elemBRA{j}{i}{1}(p,4)  = str2double(extract(3));
                elemBRA{j}{i}{1}(p,5)  = str2double(extract(4));
                elemBRA{j}{i}{1}(p,6)  = str2double(extract(5))*10^-2; % cm to m
                elemBRA{j}{i}{1}(p,7)  = str2double(extract(6));
