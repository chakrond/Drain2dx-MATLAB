...
for j=1:length(File)
    ...
    for i=1:nele
        ...
        keyword     = {' HISTORY FOR ELEMENT   4 OF ELEMENT GROUP   1, ANALYSIS SEGMENT   2'};
        toggle      = true;
        ...
        while toggle
            ...
            if length(aline)>=length(char(keyword))
                if length(aline)>0 && strcmpi(aline(1:length(char(keyword))),keyword)
                    ...
                    extract     = regexp(aline,exp,'match');
                    % Node 1
                    elemC{j}{i}{1}(1,3)  = str2double(extract(8));
                    % Node 2
                    elemC{j}{i}{2}(1,3)  = str2double(extract(9));
                    for k=1:4
                        aline   = fgetl(opf);
                    end
                    toggle    = false;
        ...
        for p=1:nstp
            ...
            if length(aline)>0
                ...
                elemC{j}{i}{1}(p,2)  = str2double(extract(2))*scalef;
                elemC{j}{i}{1}(p,3)  = elemC{j}{i}{1}(1,3);
                elemC{j}{i}{1}(p,4)  = str2double(extract(4));
                elemC{j}{i}{1}(p,5)  = str2double(extract(5))*10^-2; % kN-cm to kN-m
            ...
            elemC{j}{i}{2}(p,5)  = str2double(extract(3))*10^-2; % kN-cm to kN-m
