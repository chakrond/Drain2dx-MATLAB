...
for k=Bid(s) %:nUB
    ...
    for j=Did(s)
        ...
        % j=1;
        resuid          = [num2str(q,'R%01d') num2str(k,'B%02d') num2str(j,'D%02d')];
...
run NL_Elem_Gen_Extract.m
run NL_Extract_Beam.m
run NL_Extract_Brace.m
run NL_Extract_Column.m
