...
% Plot Enveloped Bending Diagram
...
keyword    = '*NODECOORDS';
keyword2   = '!END';
cinfo      = 2; % 2 for bendind, 3 for shear, 4 for axial
SRSScinfo  = 8; % 8 for bending, 5 for axial
...
%     [r4,c4]       = find(colp{i}(:,1) == corcol2 );    
    ...
    [r3]          = max( colp{i}(1:2,cinfo) );
    [r4]          = max( colp{i}(3:4,cinfo) );
    
    if max(abs( colp{i}(1:2,cinfo) ))>0 && max( colp{i}(1:2,cinfo) ) == 0
        [r3]          = min( colp{i}(1:2,cinfo) );
    end 
    if max(abs( colp{i}(3:4,cinfo) ))>0 && max( colp{i}(3:4,cinfo) ) == 0
        [r4]          = min( colp{i}(3:4,cinfo) );
    end
    ...
    text( [coord(r1,2)+(r3/sc), coord(r2,2)-(r4/sc)]... 
        , [coord(r1,3), coord(r2,3)]...
        , {num2str(round(r3)), num2str(round(-r4))} );
...
for i=1:ncol
    ...
    [r3]          = max( abs(colp{i}(1:2,2)) );
    [r4]          = max( abs(colp{i}(3:4,2)) );    
    ...
    text( [coord(r1,2)+(r3/sc)+(sct*McolSRSS(i,SRSScinfo)/sc), coord(r2,2)-(r4/sc)-(McolSRSS(i,SRSScinfo)/sc)]... 
        , [coord(r1,3), coord(r2,3)]...
        , {num2str(round(r3+McolSRSS(i,SRSScinfo))), num2str(round(-r4-McolSRSS(i,SRSScinfo)))} );
